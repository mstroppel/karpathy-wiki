...
function validateJsonValue(value, seen = new Set()) {
  ...
  seen.add(value)
  for (const entry of Array.isArray(value) ? value : Object.values(value)) validateJsonValue(entry, seen)
  seen.delete(value)
}
...
function validatePendingItems(result, sourceRoot, wikiSourceRoot) {
  ...
  for (const status of ["new", "outdated", "current"]) {
    for (const item of result[status]) {
      if (!item || typeof item !== "object") throw new Error(`${status} enthält keinen gültigen Eintrag`)
      if (typeof item.source_key !== "string" || !item.source_key) throw new Error(`${status} enthält keinen source_key`)
      if (keys.has(item.source_key)) throw new Error(`source_key ist mehrfach vorhanden: ${item.source_key}`)
      ...
      if (!REVISION_RE.test(item.source_revision)) throw new Error(`${item.source_key}: source_revision ist ungültig`)
      if (!item.frontmatter || typeof item.frontmatter !== "object" || Array.isArray(item.frontmatter)) {
...
export async function scanIngestStatus({ sourceRoot, wikiSourceRoot, adapterRoot, includeCurrent = false }) {
  ...
  for (const entry of sourceDirectories) {
    ...
    try {
      ...
      if (typeof adapter.default !== "function") throw new Error("Adapter exportiert keine Statusfunktion")
      const result = finalize(await adapter.default({
        sourceRoot: adapterSourceRoot,
        wikiSourceRoot,
      }), adapterSourceRoot, wikiSourceRoot)
