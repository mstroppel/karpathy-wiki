...
- Lies vor schreibenden Paperless-Vorgängen `/knowledge/sources/paperless/revoked.md` und bereinige widerrufenes Wissen nur auf ausdrücklichen Auftrag.'
  paperless_structure='
- `sources/<von>-<bis>/`: Paperless-Zusammenfassungen in Bereichen von jeweils 1000 IDs; bewahre `paperless_id`, `paperless_url` und `source_revision` aus der anonymisierten Quelle und rendere den Paperless-Link sichtbar in die Quellenseite.'
fi
